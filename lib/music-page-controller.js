...
export function createMusicPageController(options = {}) {
  ...
  function observeMotionGate(result) {
    const reason = result.accepted ? 'QUEUED' : result.reason;
    if (!['QUEUED', 'WAIT_FOR_RETURN', 'WAIT_FOR_OUTWARD', 'REARMED', 'REARMED_DWELL'].includes(reason)) {
      return;
    }
    if (reason === lastMotionGateReason) return;
    lastMotionGateReason = reason;
    log('log', 'MUSIC_GESTURE_GATE', { reason });
  }
