...
export function createMusicEngine(options = {}) {
  ...
  return {
    ...
    armed: true,
    centerCandidateSince: null,
    cyclePeakExcursion: null,
    cycleReturnFloor: null,
    pendingByGrid: {},
    sequence: 0,
    config
  };
...
function excursionFromPose(pose) {
  return Math.max(Math.abs(pose.yaw), Math.abs(pose.pitch));
}

function advanceReturnCycle(engine, excursion) {
  const priorPeak = Number.isFinite(engine.cyclePeakExcursion)
    ? engine.cyclePeakExcursion
    : excursion;
  const peak = Math.max(priorPeak, excursion);
  let floor = Number.isFinite(engine.cycleReturnFloor)
    ? engine.cycleReturnFloor
    : null;
  if (floor == null && priorPeak - excursion >= engine.config.rearmDeadZoneDeg) {
    floor = excursion;
  } else if (floor != null) {
    floor = Math.min(floor, excursion);
  }
  return {
    engine: {
      ...engine,
      cyclePeakExcursion: peak,
      cycleReturnFloor: floor
    },
    ready: floor != null && excursion - floor >= engine.config.deadZoneDeg
  };
}
...
export function proposeMotion(engine, motion) {
  ...
  const excursion = excursionFromPose(pose);
  ...
  if (zone == null) {
    ...
    const centerCandidateSince = engine.centerCandidateSince ?? timestampMs;
    const settledNearCenter = Number.isFinite(timestampMs) && Number.isFinite(centerCandidateSince) &&
      timestampMs - centerCandidateSince >= engine.config.rearmDwellMs;
    const rearmed = engine.armed || withinRearm || settledNearCenter;
    const cycle = engine.armed ? { engine } : advanceReturnCycle(engine, excursion);
    return {
      engine: {
        ...cycle.engine,
        armed: rearmed,
        centerCandidateSince: rearmed ? null : centerCandidateSince,
        cyclePeakExcursion: rearmed ? null : cycle.engine.cyclePeakExcursion,
        cycleReturnFloor: rearmed ? null : cycle.engine.cycleReturnFloor
      },
      accepted: false,
      reason: withinRearm
        ? 'REARMED'
  ...
  let motionEngine = engine;
  if (!engine.armed) {
    const cycle = advanceReturnCycle(engine, excursion);
    motionEngine = { ...cycle.engine, centerCandidateSince: null };
    if (!cycle.ready) {
      return {
        engine: motionEngine,
        accepted: false,
        reason: motionEngine.cycleReturnFloor == null ? 'WAIT_FOR_RETURN' : 'WAIT_FOR_OUTWARD'
      };
    }
  }
  if (timestampMs - motionEngine.lastAcceptedAt < motionEngine.config.cooldownMs) {
    return {
      engine: motionEngine,
      accepted: false,
      reason: 'COOLDOWN'
  ...
  const dueAt = quantizeGrid(timestampMs - motionEngine.startedAt, motionEngine.config) + motionEngine.startedAt;
  const gridIndex = Math.round((dueAt - motionEngine.startedAt) / motionEngine.config.gridMs);
  const harmonyGridIndex = Math.floor((dueAt - motionEngine.startedAt + 1e-9) / motionEngine.config.harmonyGridMs);
  const harmony = harmonyAtGrid(harmonyGridIndex, motionEngine.config);
  const candidate = {
    id: motionEngine.sequence + 1,
    timestampMs,
    dueAt,
    gridIndex,
    harmonyGridIndex,
  ...
  const existing = motionEngine.pendingByGrid[key];
  ...
  const pendingByGrid = { ...motionEngine.pendingByGrid, ...(wins ? { [key]: candidate } : {}) };
  const next = {
    ...motionEngine,
    pendingByGrid,
